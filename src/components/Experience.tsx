...
const experiences = [
  {
    ...
    description: "Leading development of scalable cloud solutions with AI integration, serving 100K+ users.",
    skills: ["React", "Node.js", "AWS", "AI/ML Integration"],
    color: "neon-cyan",
  },
  {
    icon: Code2,
    ...
    description: "Built enterprise applications with modern tech stack, improving performance by 60%.",
    skills: ["TypeScript", "PostgreSQL", "Docker", "GraphQL"],
    color: "pastel-blue",
  },
  {
    icon: Zap,
    ...
    description: "Crafted pixel-perfect interfaces and seamless user experiences for premium clients.",
    skills: ["React", "Tailwind", "Animation", "UI/UX"],
    color: "neon-magenta",
  },
  {
    icon: Briefcase,
    ...
    company: "StartupHub",
    description: "Started the journey building MVPs and learning the fundamentals of modern web development.",
      ...
      <div className="max-w-6xl mx-auto relative">
        <div className="text-center mb-12 sm:mb-16 lg:mb-20 px-2">
          ...
          <p className="text-base sm:text-lg lg:text-xl text-muted-foreground max-w-2xl mx-auto leading-relaxed">
            A timeline of growth, innovation, and problem-solving across diverse tech landscapes
          </p>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 z-10">
                    <div className={`w-12 h-12 lg:w-16 lg:h-16 rounded-full bg-card border-2 border-${exp.color} flex items-center justify-center shadow-lg hover:shadow-glow-cyan active:shadow-glow-cyan transition-all duration-300 group-hover:scale-110 group-active:scale-105`}>
                      <Icon className={`w-6 h-6 lg:w-7 lg:h-7 text-${exp.color}`} />
                    </div>
                  </div>

                  {/* Content Card - Mobile Optimized */}
                  <div className={`flex-1 ${isEven ? 'md:pr-12 md:text-right' : 'md:pl-12 md:text-left'} text-left w-full`}>
                    <div className="bg-card/50 backdrop-blur-sm border border-border/50 rounded-xl sm:rounded-2xl p-5 sm:p-6 lg:p-8 hover:border-neon-cyan/50 active:border-neon-cyan/70 transition-all duration-500 hover-lift hover-glow touch-manipulation">
                      <div className="flex items-center gap-3 mb-3 sm:mb-4 justify-start md:justify-start">
                        <div className="md:hidden">
                          <Icon className={`w-5 h-5 sm:w-6 sm:h-6 text-${exp.color}`} />
                        </div>
                        <span className="text-xs sm:text-sm font-mono text-muted-foreground">{exp.year}</span>
                      </div>
                      
                      <h3 className="text-xl sm:text-2xl font-bold mb-2 text-foreground">{exp.title}</h3>
                      <p className={`text-base sm:text-lg text-${exp.color} mb-3 sm:mb-4`}>{exp.company}</p>
